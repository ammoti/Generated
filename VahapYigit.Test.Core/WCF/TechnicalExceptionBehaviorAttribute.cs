...
namespace VahapYigit.Test.Core
{
	...
	public sealed class TechnicalExceptionBehaviorAttribute : Attribute, IServiceBehavior, IEndpointBehavior, IContractBehavior
	{
		...
		{
			Debug.WriteLine(string.Format("Applying client TechnicalExceptionBehaviorAttribute to contract {0}", contract.ContractType));
		...
		{
			Debug.WriteLine(string.Format("Applying dispatch TechnicalExceptionBehaviorAttribute to contract {0}", contract.ContractType.FullName));
		...
		{
			Debug.WriteLine(string.Format("Applying client TechnicalExceptionBehaviorAttribute to endpoint {0}", endpoint.Address));
			this.ApplyClientBehavior(runtime);
		}

		void IEndpointBehavior.ApplyDispatchBehavior(ServiceEndpoint endpoint, EndpointDispatcher dispatcher)
		{
			Debug.WriteLine(string.Format("Applying dispatch TechnicalExceptionBehaviorAttribute to endpoint {0}", endpoint.Address));
		...
		{
			Debug.WriteLine(string.Format("Applying dispatch TechnicalExceptionBehaviorAttribute to service {0}", service.ServiceType.FullName));
		...
		{
			if (!runtime.MessageInspectors.Any(i => i is TechnicalExceptionMessageInspector))
			{
				runtime.MessageInspectors.Add(new TechnicalExceptionMessageInspector());
			}
		}

		private void ApplyDispatchBehavior(ChannelDispatcher dispatcher)
		{
			if (!dispatcher.ErrorHandlers.Any(i => i is TechnicalExceptionErrorHandler))
			{
				dispatcher.ErrorHandlers.Add(new TechnicalExceptionErrorHandler());
			}
		}
