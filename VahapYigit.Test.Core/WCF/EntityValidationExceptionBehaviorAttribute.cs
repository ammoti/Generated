...
namespace VahapYigit.Test.Core
{
	...
	public sealed class EntityValidationExceptionBehaviorAttribute : Attribute, IServiceBehavior, IEndpointBehavior, IContractBehavior
	{
		...
		{
			Debug.WriteLine(string.Format("Applying client EntityValidationExceptionBehaviorAttribute to contract {0}", contract.ContractType));
		...
		{
			Debug.WriteLine(string.Format("Applying dispatch EntityValidationExceptionBehaviorAttribute to contract {0}", contract.ContractType.FullName));
		...
		{
			Debug.WriteLine(string.Format("Applying client EntityValidationExceptionBehaviorAttribute to endpoint {0}", endpoint.Address));
			this.ApplyClientBehavior(runtime);
		}

		void IEndpointBehavior.ApplyDispatchBehavior(ServiceEndpoint endpoint, EndpointDispatcher dispatcher)
		{
			Debug.WriteLine(string.Format("Applying dispatch EntityValidationExceptionBehaviorAttribute to endpoint {0}", endpoint.Address));
		...
		{
			Debug.WriteLine(string.Format("Applying dispatch EntityValidationExceptionBehaviorAttribute to service {0}", service.ServiceType.FullName));
		...
		{
			if (!runtime.MessageInspectors.Any(i => i is EntityValidationExceptionMessageInspector))
			{
				runtime.MessageInspectors.Add(new EntityValidationExceptionMessageInspector());
			}
		}

		private void ApplyDispatchBehavior(ChannelDispatcher dispatcher)
		{
			if (!dispatcher.ErrorHandlers.Any(i => i is EntityValidationExceptionErrorHandler))
			{
				dispatcher.ErrorHandlers.Add(new EntityValidationExceptionErrorHandler());
			}
		}
